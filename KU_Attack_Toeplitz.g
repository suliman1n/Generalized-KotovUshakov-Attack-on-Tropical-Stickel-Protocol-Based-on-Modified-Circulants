...
PowersMinPlus := function(A, alpha, s)
  ...
  if s=1 then
  	return List([1..n], i -> List([1..n], function(j) 
      		if i <= j and (i-j) mod n = (n-alpha) mod n and alpha<n then
            		return 0;
...
CreateUpperToeplitz := function(n, mm)
  return List([1..n], i -> List([1..n], function(j) 
if i=j then
  return 0;
else
...
CreateUpperToeplitz := function(column)
    local n, circulant_matrix, i, j;
    ...
    for i in [1..n] do
        for j in [1..n] do
            circulant_matrix[i][j] := column[((i - j) mod n + 1)];
        od;
    od;
...
CreateLowerToeplitz := function(column)
    local n, circulant_matrix, i, j;
...
TestAttack := function(n, D, pm, pM, ApplyAttack,s,t)
  ...
  A1 := CreateUpperToeplitz(RandomColumnVector(n,pm,pM));
  A2 := CreateLowerToeplitz(RandomColumnVector(n,pm,pM));

  B1 := CreateUpperToeplitz(RandomColumnVector(n,pm,pM));
  B2 := CreateLowerToeplitz(RandomColumnVector(n,pm,pM));
